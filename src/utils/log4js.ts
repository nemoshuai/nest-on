...
Log4js.addLayout('nest-in', (logConfig: any) => {
  return (logEvent: Log4js.LoggingEvent): string => {
    ...
    const msgOutput: string = msgList.join(' ');
    ...
    const moduleOutput: string = moduleName
      ...
      : '[LoggerService] ';
    let levelOutput = `[${logEvent.level}] ${msgOutput}`;
...
export class Logger {
  ...
  static getStackTrace(deep = 2): string {
    ...
    const basename: string = Path.basename(fileName as string);
